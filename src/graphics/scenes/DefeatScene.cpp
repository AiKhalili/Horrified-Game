...
{
    background = TextureManager::getInstance().getOrLoadTexture("DefeatScene", "assets/images/background/defeat_scene.png");
...
void DefeatScene::showErrorMessage(const std::string &msg)
{
    if (!errorLabel)
        return;
...
void DefeatScene::createLabels()
{
    const char *text = "Defeat Scene";
    int fontSize = 75;
...
void DefeatScene::createButtons()
{
    ...
    saveBtn->setOnClick([this]()
                        {
                            ...
                            SaveManager::getInstance().saveGameToSlot("DefeatScene"); 
    ...
    auto pickBtn = std::make_unique<UIButton>(Rectangle{centerX - buttonWidth / 2, topY + 3 * (gapY + buttonHeight), buttonWidth, buttonHeight},
                                              "Defeat", fontsize, labelcolor, textcolor, clickcolor2, textcolor);
    ...
    try
    {
        ...
        Game::getInstance().handleDefeat(monselect, itemselect);
