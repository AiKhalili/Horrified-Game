...
public:
    ...
    void saveGameToSlot(int, const Game&); // ذخیره کامل بازی با شماره اسلات
    ...
    void saveCurrentHeroStateToFile(const std::string& filename, int currentIndex, int actionsLeft);
    std::pair<int, int> loadCurrentHeroStateFromFile(const std::string& filename);
    ...
    void savePerkDeckToFile(const std::string& filename, const std::vector<PerkCard>& deck);
    std::vector<PerkCard> loadPerkDeckFromFile(const std::string& filename);
